...
def _session(user, date, status, score=None, total=None):
    return DailySession.objects.create(
        user=user, date=date, status=status, score=score, total=total
    )
...
def test_dashboard_rest_day_not_missed():
    user = User.objects.create(first_name="Kid")
    today = timezone.localdate()
    rest = today.weekday()  # make only today's weekday a study day → others are rest days
    LearningProfile.objects.create(user=user, study_weekdays=[rest])
    _session(user, today - datetime.timedelta(days=7), DailySession.Status.COMPLETED)
    d = build_dashboard(user)
